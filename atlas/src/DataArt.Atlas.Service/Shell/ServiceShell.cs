...
using DataArt.Atlas.Service.Application.Logging;
using DataArt.Atlas.Service.Application.Messaging;
using DataArt.Atlas.Service.Application.Owin;
using DataArt.Atlas.Service.Application.Swagger;
using DataArt.Atlas.Service.Application.WebApi;
using DataArt.Atlas.Service.HealthCheck;
using DataArt.Atlas.Service.VersionInfo;
    ...
    public abstract class ServiceShell : IApplication 
    {
        ...
        public async Task StartAsync(string applicationUrlOverride, CancellationToken cancellationToken)
        {
            try
            {
                ...
                mainThreadCorrelationId = CorrelationContext.CorrelationId.Value;
        ...
        private async Task StartStartablesAsync(CancellationToken cancellationToken)
        {
            ...
            foreach (var startable in ApplicationContainer.Resolve<IEnumerable<Startable>>())
            {
                awaitList.Add(startable.StartAsync(cancellationToken));
            }
        ...
        private async Task StopStartablesAsync()
        {
            ...
            foreach (var startable in ApplicationContainer.Resolve<IEnumerable<Startable>>())
            {
                awaitList.Add(startable.StopAsync());
            }
